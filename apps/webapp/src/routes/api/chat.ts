...
Important: When you use any tool, do not expose tool outputs in the chat. Only log details on the server console. After executing any tool, respond in the chat with exactly: "ok".`,
    ...
    tools: {
      ...
      readWorkflowContent: tool({
        description: 'Return the current workflow content. Use when the user asks to read the workflow.',
        inputSchema: z.object({}),
        execute: async () => {
          const fakeContent = 'Este workflow dice: papas fritas con helado, nada que ver.';
          console.log('[Tool: readWorkflowContent] ->', {
      ...
      editWorkflowContent: tool({
        description: 'Edit the current workflow content. Use when the user asks to modify the workflow.',
        inputSchema: z.object({ content: z.string().min(1) }),
        execute: async ({ content }: { content: string }) => {
          console.log('[Tool: editWorkflowContent] ->', {
            workflowId: workflowId || 'none',
            newContent: content,
          });
          return { ok: true };
