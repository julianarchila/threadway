...
export default function PhoneMockup() {
  const messages: ChatMessage[] = React.useMemo(
    () => [
      { id: "m1", from: "user", type: "text", text: "Hey", time: "09:41", delivered: true },
      {
        id: "m2",
        from: "bot",
        type: "text",
        text:
          "Welcome back! Your workflows are ready: Spending tracker, Reading list, Workouts, Reminders. Send a message and I'll handle it.",
        time: "09:41",
      },
      { id: "m3", from: "user", type: "text", text: "Bought lunch $12", time: "09:42", delivered: true },
      {
        id: "m4",
        from: "bot",
        type: "text",
        text: "Logged: Lunch • $12 → Sheets: Spending (Today). Want a weekly summary?",
        time: "09:42",
      },
      { id: "m5", from: "user", type: "text", text: "Save https://example.com/atomic-habits", time: "09:42", delivered: true },
      {
        id: "m6",
        from: "bot",
        type: "link",
        link: {
          href: "https://www.notion.so/your-reading-list",
          title: "Saved to Notion",
          host: "notion.so",
          label: "Reading list",
        },
        helper: "Open in Notion",
        time: "09:42",
      },
      { id: "m7", from: "user", type: "text", text: "Pushups 20", time: "09:43", delivered: true },
      { id: "m8", from: "bot", type: "text", text: "Logged: Pushups • 20 reps → Workouts log.", time: "09:43" },
      { id: "m9", from: "user", type: "voice", text: "Remind me to call mom Friday 6pm.", duration: "0:06", time: "09:44", delivered: true },
      { id: "m10", from: "bot", type: "text", text: "Reminder set for Fri 6:00 PM. I'll message you then.", time: "09:44" },
    ],
    []
  )
                    ...
                    >
                      ...
                      >
                        ...
                        <div className="relative px-3.5 py-2.5">
                          ...
                          {m.type === "link" && (
                            <LinkCard link={(m as { link: ChatLink; helper?: string }).link} helper={(m as { link: ChatLink; helper?: string }).helper} />
                          )}

                          {m.type === "voice" && (
                            <VoiceNote text={(m as { text: string; duration?: string }).text} duration={(m as { text: string; duration?: string }).duration} inverted={isUser} />
                          )}

                          {m.type === "list" && (
                            <div className="space-y-2">
                              <p className="tracking-[0.005em]">{(m as { text: string; items: ListItem[] }).text}</p>
                              <div className="mt-1 grid gap-2">
                                {(m as { text: string; items: ListItem[] }).items.map((item: ListItem, i: number) => (
                                  <a
                                    key={i}
                                    href={item.href}
                                    target="_blank"
                          ...
                          <div className={cn("mt-1 flex items-center gap-1 text-[10px]")}>
                            ...
                            {isUser && (m as { delivered?: boolean }).delivered && (
